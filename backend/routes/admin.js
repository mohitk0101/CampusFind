...
// @GET /api/admin/posts - Get all posts by status for admin
...
// @PUT /api/admin/posts/:id/activate - Activate an archived post
router.put('/posts/:id/activate', protect, adminOnly, async (req, res) => {
  try {
    const post = await Post.findById(req.params.id);
    if (!post) return res.status(404).json({ success: false, message: 'Post not found.' });
    if (post.status !== 'archived') {
      return res.status(400).json({ success: false, message: 'Only archived posts can be activated.' });
    }

    post.status = 'active';
    post.archivedAt = null;
    post.warnedAboutArchiving = false;
    await post.save();
    ...
    await Notification.create({
      user: post.reporter,
      post: post._id,
      message: `Your archived post for "${post.itemName}" has been reactivated by the admin!`,
      type: 'post_approved'
    });

    res.json({ success: true, post, message: 'Post activated successfully.' });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
